...
public class ProCLanguage extends AbstractCLikeLanguage {
	...
	public int getLinkageID() {
		// memo - getLinkageID
		// C_LINKAGE_IDã?§ã?ªã?„ã?¨ã€?PreProcessorã?Œincludeã‚’å±•é–‹ã?—ã?¦ã??ã‚Œã?ªã?„.
		// CPP_LINKAGE_ID ã?§ã‚‚ã? ã‚?.
...
//		return ILinkage.CPP_LINKAGE_ID;
	...
	protected IScanner mycreateScanner(
		...
		/*
		 * gccã?®ãƒ—ãƒ­ã‚¸ã‚§ã‚¯ãƒˆè¨­å®š(include path,ãƒžã‚¯ãƒ­å®šç¾©etc)ã‚’å…ƒã?«
		 * ScannerInfoã‚’ä½œæˆ?ã?—ã?¦ProCPreprocessorã?«æ¸¡ã?™
		 */
